 ...
 *
 */
class PostRequirement extends AbstractProspect
{
	public function saveProspect()
	{
		...
		$attributes = array(
				...
				"MinimumArea"=>$_GET['SAreaMin'],
				"MaximumArea"=>$_GET['SAreaMax']);

		// create the associative array representing the interaction details.
		$interaction = array("Type"=>"PostRequirement","IP"=>VitruUtils::getIpAddr(),"Attributes"=>$attributes);
		...
		// Send the SMS
		VitruSmsManager::sendSms('Thank you for visiting our website! Our customer care executive shall get in touch with you shortly. Regards, Vitruvian Tech');

		// Send the Email
		PostRequirement::sendEmail($response);

		echo '<a href="'.PostRequirement::getBackUrl().'"><img src="images/vitru-images/back_btn.gif" border="0"/></a>';
		echo '<br></br>';
	...
	{
		return 'index.php?option=com_postrequirement&view=basic&' . VitruUtils::getQueryString(array('task','view','option'));
	}
	...
	private static function sendEmail($response)
	{
		...
		// form the subject
		$subject = $_GET['Name'].' Posted this Requirement !';

		// form the body.
		$body .= '<html>';
		$body .= '<body style="font-size: 11; font-family: Verdana;">';
		$body .= '<h2>Requirement Posted !</h2><br>';
		...
		$body .= '<tr><td>MinPrice:</td><td>'.$response['CostDownside'].'</td></tr>';
		$body .= '<tr><td>MaxPrice:</td><td>'.$response['CostUpside'].'</td></tr>';
		$body .= '<tr><td>SAreaMin:</td><td>'.$response['TotalAreaDownside'].'</td></tr>';
		$body .= '<tr><td>SAreaMax:</td><td>'.$response['TotalAreaUpside'].'</td></tr>';
		$body .= '</table>';
		$body .= '<br></br>';
		$body .= 'Regards';
		$body .= '<br></br>';
